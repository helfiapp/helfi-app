...
export default function RecommendedMealClient() {
  ...
  const generate = async () => {
    ...
    setError(null)
    try {
      // If this was a one-shot generate triggered via ?generate=1, strip the flag so
      // navigating back doesn't re-trigger generation (and spend credits again).
      try {
        if (generateFlag) {
          const qs = new URLSearchParams()
          qs.set('date', date)
  ...
  useEffect(() => {
    ...
    generate()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [generateFlag, loadingContext, seenExplain])
