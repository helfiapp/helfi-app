...
export default function RecommendedMealClient() {
  ...
  const mealMacroRows = useMemo(
    () => [
      { label: 'Calories', value: formatMacroValue(draftTotals.calories, 'kcal'), valueClass: 'text-gray-900' },
      { label: 'Protein', value: formatMacroValue(draftTotals.protein_g, 'g', 1), valueClass: 'text-rose-600' },
      { label: 'Carbs', value: formatMacroValue(draftTotals.carbs_g, 'g', 1), valueClass: 'text-emerald-600' },
      { label: 'Fat', value: formatMacroValue(draftTotals.fat_g, 'g', 1), valueClass: 'text-indigo-600' },
      { label: 'Fiber', value: formatMacroValue(draftTotals.fiber_g, 'g', 1), valueClass: 'text-cyan-600' },
      { label: 'Sugar', value: formatMacroValue(draftTotals.sugar_g, 'g', 1), valueClass: 'text-orange-600' },
    ],
    [draftTotals],
  )
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-200">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-lg font-semibold text-gray-900 break-words">{pageTitle}</div>
              ...
              {active ? (
                <>
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white p-4">
                    ...
                    <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {mealMacroRows.map((row) => (
                        <div key={row.label} className="rounded-xl border border-gray-100 bg-gray-50 px-3 py-2">
                          <div className="text-xs text-gray-500">{row.label}</div>
                          <div className={`text-sm font-semibold ${row.valueClass}`}>{row.value}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white">
                    <div className="flex items-center gap-2 border-b border-gray-200 px-3 pt-3">
                      {[
