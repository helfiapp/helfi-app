...
function buildFoodSystemPrompt(foodDiarySnapshot: FoodDiarySnapshot | null): string {
  ...
  return [
    ...
    'Each option must be a practical recipe meal (not just a food name).',
    ...
    '[[MEAL_OPTIONS_JSON]]',
    '{"options":[{"optionNumber":1,"title":"...","category":"breakfast|lunch|dinner|snacks|uncategorized","servings":1,"prepMinutes":10,"cookMinutes":15,"ingredients":["..."],"steps":["..."]},{"optionNumber":2,"title":"...","category":"breakfast|lunch|dinner|snacks|uncategorized","servings":1,"prepMinutes":10,"cookMinutes":15,"ingredients":["..."],"steps":["..."]}]}',
    '[[/MEAL_OPTIONS_JSON]]',
    'Rules for JSON block:',
    '- valid JSON only, double quotes only, no markdown code fences',
    '- include both options shown in the visible answer',
    '- each option must include at least 3 ingredients and 3 steps',
...
export async function POST(req: NextRequest) {
  try {
    ...
    const foodContextFromThread = isFoodChat && threadId ? await getThreadFoodContext(threadId) : null
    if (isFoodChat && threadId && foodContextOverride) {
      await updateThreadFoodContext(threadId, foodContextOverride.slice(0, 1200))
    }
    const effectiveFoodContext = foodContextOverride || foodContextFromThread
    if (isFoodChat && effectiveFoodContext) {
      systemPrompt += [
        '',
        'RECENT PHOTO CONTEXT (IMPORTANT):',
        effectiveFoodContext,
