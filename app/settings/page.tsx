                      ...
                      {isInstalled && (
                        <a
                          ...
                          onClick={(e) => {
                            ...
                            // If the URL scheme doesn't work, prevent default and show message
                            try {
                              // Attempt to navigate - if this fails, iOS will handle it
                              window.location.href = 'app-settings:'
                            } catch (err) {
                              // If URL scheme fails, try alternative
                              e.preventDefault()
                              try {
                                window.open('app-settings:', '_blank')
                              } catch (e2) {
                                // If all methods fail, user needs to manually open Settings
                                // Don't show alert - instructions are already visible above
                              }
                            }
                          }}
                          className="w-full mt-4 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 shadow-md no-underline"
                        >
