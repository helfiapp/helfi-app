...
export const shouldSkipRepeatedWrite = async ({
  userId,
  scope,
  payloadHash,
  windowMs,
}: WriteGuardInput): Promise<WriteGuardResult> => {
  if (!userId || !scope || !payloadHash) return { skip: false, hitCount: 0 }
  await ensureWriteGuardTable()
  try {
    const rows: any[] = await prisma.$queryRawUnsafe(
      'SELECT id, payloadHash, lastSeenAt, hitCount FROM WriteGuard WHERE userId = $1 AND scope = $2',
      userId,
      scope
    )
    ...
    if (existing && existing.payloadHash === payloadHash) {
      ...
      if (lastSeenAt && now - lastSeenAt < windowMs) {
        ...
        return { skip: true, hitCount: Number(existing.hitCount || 0) + 1 }
      }
    }
