...
- Current cache-busted web/PWA copies use the `-v20260721` suffix, with the approved native-quality 1024px Apple touch icon at `public/apple-touch-icon-v20260722-1024.png`. They must remain byte-for-byte renders of the protected outputs above; the changed filenames prevent iOS from reusing the old blurry icon.
...
- `app/layout.tsx` → manifest link + icons point at the current local cache-busted copies: `/manifest-v20260722.json`, `/icons/app-192-v20260721.png`, `/icons/app-512-v20260721.png`, and `/apple-touch-icon-v20260722-1024.png`. The 1024px Apple asset must also keep the explicit `apple-touch-icon-precomposed` link so iOS uses the approved artwork without softening it.
