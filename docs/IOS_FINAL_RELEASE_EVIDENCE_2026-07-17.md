...
| Connection | Open, Connecting, Listening, understandable error, retry | Physical iPhone 14 Pro Max / builds 29-30 | RETEST REQUIRED | Build 29 failed at `Contacting Helfi voice` because the app stopped the whole connection after 8 seconds. Commit `0c3dd4fe` moved the voice route to Sydney, reduced server waiting, and split the cutoff into server-answer and media-finish checks. Build 30 is installed locally. | Confirmed build 29 app/backend connection defect repaired in build 30 | Owner must speak on the unlocked phone while mirroring is disconnected |
...
| Local build 30 | Build and install the connection repair without TestFlight | Physical iPhone 14 Pro Max | PASS - INSTALLED | `xcrun devicectl` reports Helfi 1.0.0 build 30 installed from commit `0c3dd4fe`; live deployment is READY and voice readiness passes | No TestFlight upload or App Store review action was taken | Direct owner microphone/loudspeaker retest required |
