...
export default function HeroCarousel() {
  ...
  // Mobile: Smooth scroll animation - hold for 4 seconds, then quickly flash to next
  useEffect(() => {
    if (!isMobile) return
    ...
    const advanceSlide = () => {
      ...
      // Smooth scroll from right to left
      container.scrollTo({ 
        left: scrollPosition, 
        behavior: 'smooth' 
      })
      ...
      }, 500) // Smooth scroll takes ~300-500ms

      // Hold for 4 seconds before next transition
      advanceTimeoutId = setTimeout(advanceSlide, 4000)
    }
    ...
    // Start the cycle after 4 seconds (hold first slide for 4 seconds)
    advanceTimeoutId = setTimeout(advanceSlide, 4000)
