...
- ⚠️ Copy-to-Today duplication bug: copying a past-day entry into today sometimes produces two copies with the same timestamp (see latest screenshot). Investigate in `app/food/page.tsx` around the duplicate/copy flow and dedupe logic—ensure a single insert per action and that dedupe keys handle missing `localDate` and newly assigned ids. Do not weaken existing guard rails; fix by preventing double insertion (e.g., avoid multiple state updates/save paths firing) and tightening dedupe so same description/time/date collapses to one row.
